<template>
  <v-sheet border rounded :height="300" color="surface">
    ...
    >
      ...
      <template #tag_amt="row">
        <span class="font-weight-bold" v-if="!row.value.tag_full_toggle">
          {{ formatCurrency(row.value.tag_amt) }}
        </span>
        <span class="font-weight-bold font-italic" v-else>(Full)</span>
      </template>
    </vue3-datatable>
  </v-sheet>
</template>
