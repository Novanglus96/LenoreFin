            ...
            <v-stepper-window-item value="1">
              <v-banner>
                <v-banner-text>
                  ...
                  like to import. Click
                  <a href="/example.csv">here</a>
                  for an example .csv file to download.
                </v-banner-text>
              </v-banner>
              <v-file-input
                :clearable="false"
                  ...
                  <v-row>
                    ...
                    >
                      <v-sheet border>
                        <v-container>
                          <v-row dense>
                            <v-col>{{ item.raw.file_status }}</v-col>
                          </v-row>
                          <v-row dense>
                  ...
                  <v-row>
                    ...
                    >
                      <v-sheet border>
                        <v-container>
                          <v-row dense>
                            <v-col>{{ item.raw.file_type }}</v-col>
                          </v-row>
                          <v-row dense>
                            <v-col class="text-center text-caption font-bold">
                              will be mapped to
                  ...
                  <v-row>
                    ...
                    >
                      <v-sheet border>
                        <v-container>
                          <v-row dense>
                            <v-col>{{ item.raw.file_account }}</v-col>
                          </v-row>
                          <v-row dense>
                            <v-col class="text-center text-caption font-bold">
                              will be mapped to
  ...
  const parseCSV = csvFile => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      for (let j = 0; j < header.length; j++) {
        ...
        if (
          (formatted_header.startsWith('"') &&
            formatted_header.endsWith('"')) ||
  ...
  const verifyTransactions = transactions => {
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (
        ...
        trans_obj.errors.push({
          text: "Missing Transaction Status",
          status: 0,
        });
      ...
      if (
        ...
        for (let sa = 0; sa < mappings.value.accounts.length; sa++) {
          if (
            mappings.value.accounts[sa].file_account ==
            transaction.SourceAccount
          ) {
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (!destination_account_exists) {
        for (let m = 0; m < accounts.value.length; m++) {
          if (
            accounts.value[m].account_name == transaction.DestinationAccount
          ) {
            destination_account = {
              file_account: transaction.DestinationAccount,
