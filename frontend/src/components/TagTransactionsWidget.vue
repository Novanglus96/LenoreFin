                ...
                <v-card v-if="!isLoading">
                    ...
                    <v-card-text>
                        <vue3-datatable 
                            ...
                            :columns="columns"
                            :loading="isLoading"
                            :totalRows="tag_transactions.details ? tag_transactions.details.length : 0"
                            :isServerMode="false"
                            pageSize="10"
                            :hasCheckbox="false"
                            :stickyHeader="true"
                        ...
                        ><!--height="280px"-->
                            ...
                            <template #tag_amount="row"><!-- eslint-disable-line -->
                                <span :class="row.value.tag_amount < 0 ? 'text-red' : 'text-green'">${{ row.value.tag_amount }}</span>
                            </template>
                            <template #transaction_description="row"><!-- eslint-disable-line -->
                                {{ row.value.transaction_description }}
                            </template>
                            <template #transaction_memo="row"><!-- eslint-disable-line -->
                                {{ row.transaction_memo }}
                            </template>
                            <template #transaction_pretty_account="row"><!-- eslint-disable-line -->
