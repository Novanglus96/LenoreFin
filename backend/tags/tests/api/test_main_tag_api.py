...
@pytest.mark.django_db
@pytest.mark.api
def test_main_tag_schema_has_slug_and_is_system(api_client, test_main_tag):
    response = api_client.get(
        f"/tags/main-tags/get/{test_main_tag.id}", headers=AUTH
    )
    ...
    assert data["slug"] == test_main_tag.slug
    assert data["is_system"] == test_main_tag.is_system
